...
const SpeakerCard = ({ blok }) => {
  ...
  return (
    <div {...storyblokEditable(blok)} className={styles.card}>
      ...
      <p className={`${shared_styles.paragraph} ${isOverflowHidden && `${styles.overflow_hidden}`}`}>{blok.description}</p>
      {blok.description.length > 200 &&
        <button onClick={() => setIsOverflowHidden(!isOverflowHidden)}>
          ...
          &ensp;
          <img alt="caret icon" className={isOverflowHidden ? "" : `${styles.caret_up}`} src="https://a.storyblok.com/f/173422/x/6bb82d64a7/caret-down.svg" />
          &emsp;
          <hr className={styles.hr_line} />
        </button>
      }
